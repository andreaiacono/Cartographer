...
class ProjectionPanelGL(GLCanvas):
    ...
    def OnDraw(self):
        ...
        lines = self.draw_projection(10.0, 10.0)
        # print(lines)
        glLineWidth(0.8)
        glClearColor(1, 1, 1, 1)
        glClear(GL_COLOR_BUFFER_BIT)

        glColor4f(0.7, 0.7, 0.99, 0.3)

        for line in lines:
            for polygon in line:
                glBegin(GL_LINE_STRIP)
                for point in polygon:
                    glVertex2d(point[0]/100-4.0, -point[1]/100+3.0)
                glEnd()
    ...
    def draw_projection(self, width, height):
        ...
        longitudes = [0]
        for meridian in range(1, (self.meridian_number) + 1):
            longitude = 180 - (meridian_spacing * meridian)
            longitudes.append(longitude)

        latitudes = []
        for parallel in range(1, self.parallel_number + 1):
            latitude = 90 - (parallel_spacing * parallel)
        ...
        if self.paint_grid:
            ...
            for lon in longitudes:
                vertices.append(self.draw_meridian(lon, True))

            for lat in latitudes:
                vertices.append(self.draw_parallel(lat, True))
        ...
        if self.paint_grid_specials:
            ...
            for tropics in (-23.5, 23.5):
                vertices.append(self.draw_parallel(tropics, True))

            # dc.SetPen(wx.Pen("dark grey", 1))
            # dc.SetPen(wx.Pen((64, 245, 255), 1))
            for circles in (-66.5, 66.5):
                vertices.append(self.draw_parallel(circles, True))
        ...
        for shape in self.shapes:
            for i in range(len(shape.parts)):
                ...
                current_x, current_y = tuple((val * self.mf) for val in self.projection.get_coords(rx1, ry1))
                current_x += self.tx
                current_y += self.ty
                index = 0
                ...
                for point in range(start_index + 1, end_index):
                    if point % self.resolution == 0:
                        ...
                        current_x, current_y = tuple((val * self.mf) for val in self.projection.get_coords(rx2, ry2))
                        current_x += self.tx
                        current_y += self.ty
    ...
    def draw_parallel(self, latitude, transform_coords):
        ...
        current_x = (current_x + self.tx)
        current_y = (current_y + self.ty)
        index = 0
    ...
    def draw_meridian(self, longitude, transform_coords):
        # computes the first point
