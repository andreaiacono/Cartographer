...
class LambertProjection(GenericProjection):
    ...
    def set_standard_parallels(self, param1, param2):
        self.phi1 = math.radians(param1)
        self.phi2 = math.radians(param2)
        self.precompute_values()

    def precompute_values(self):
        self.n = (math.log(math.cos(self.phi1) * mpmath.sec(self.phi2))) / math.log(
            math.tan(math.pi / 4 + self.phi2 / 2) * mpmath.cot(math.pi / 4 + self.phi1 / 2))
        if self.n == 0.0000:
            self.n = 0.00001
